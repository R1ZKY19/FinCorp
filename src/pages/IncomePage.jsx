...
import { Plus, ArrowDownLeft, Edit2, Trash2, Download } from 'lucide-react';
...
export function IncomePage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        error(res.message || 'Gagal menghapus pemasukan.');
      }
    } catch (err) {
  ...
  const handleExport = () => {
    ...
    exportToCSV(exportData, `data-pemasukan-${new Date().toISOString().substring(0, 10)}.csv`);
  };

  const totalIncomeAmount = transactions.reduce((acc, curr) => acc + curr.amount, 0);

  return (
    <div className="space-y-6">
      {/* Header Stat & Actions */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800/40 p-5 rounded-2xl">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <h2 className="text-2xl font-black text-emerald-950 dark:text-emerald-100 tracking-tight mt-0.5">
      ...
      <Card noPadding>
        {loading ? (
          <div className="p-12 text-center text-xs text-slate-400">Memuat data pemasukan...</div>
        ) : transactions.length === 0 ? (
          <EmptyState
            title="Belum ada data pemasukan"
            description="Catat pemasukan gaji, dividen, bonus, atau bisnis Anda untuk memantau arus kas."
      ...
      <ConfirmDialog
        ...
        message="Apakah Anda yakin ingin menghapus data pemasukan ini? Saldo rekening Anda akan disesuaikan secara otomatis."
      />
    </div>
  );
}
